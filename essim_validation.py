...
def validate_ESSIM_asset(asset, results):
    ...
    if port:
        for p in port:
            ...
            if not port_id:
                port_id = 'NO_PORT_ID'
                results.append('ERROR: Port of asset ' + asset_id + 'has no id')

            conn_to = p.get_connectedTo()
            if not conn_to:
                results.append('ERROR: Port ' + port_id + ' of asset ' + asset_id + 'is not connected')

            carr = p.get_carrier()
            if not carr:
                results.append('ERROR: Port' + port_id + ' of asset ' + asset_id + 'has no carrier')

            profile = p.get_profile()
            if profile:
                if asset_has_profile:
                    results.append('ERROR: asset ' + asset_id + ' has more than one profile')
                else:
                    asset_has_profile = True
    else:
        results.append('ERROR: Asset ' + asset.get_id() + ' has no ports')
    ...
    if ci:
        ...
        if mc:
            ...
            if value < 0 or value > 1:
                results.append('WARNING: Marginal costs for asset ' + asset_id + ' not between 0 and 1')
    ...
    if isinstance(asset, esdl.Producer):
        ...
        if not asset_has_profile:
            ...
            if power:
                if power == 0:
                    results.append('ERROR: Producer ' + asset_id + ' has power of 0')
            else:
                results.append('ERROR: Producer ' + asset_id + ' has no power specified')

        else:
            if asset_has_marginal_costs:
                results.append('ERROR: Producer ' + asset_id + ' has both a profile and marginal costs specified')
    ...
    if isinstance(asset, esdl.Consumer):
        if not asset_has_profile:
            ...
            if power:
                if power == 0:
                    results.append('ERROR: Consumer ' + asset_id + ' has power of 0')
            else:
                results.append('WARNING: Consumer ' + asset_id + ' has no power specified')
        else:
            if asset_has_marginal_costs:
                results.append('ERROR: Consumer ' + asset_id + ' has both a profile and marginal costs specified')

    if isinstance(asset, esdl.Conversion):
        power = asset.get_power()  # TODO: what is result if no power specified?
        if power:
            if power == 0:
                results.append('ERROR: Conversion ' + asset_id + ' has power of 0')
        else:
            results.append('WARNING: Conversion ' + asset_id + ' has no power specified')

        eff = asset.get_efficiency()  # TODO: what is result if no efficiency is specified?
        if eff:
            if eff == 0:
                results.append('ERROR: Conversion ' + asset_id + ' has efficiency of 0')
        else:
            results.append('WARNING: Conversion ' + asset_id + ' has no efficiency specified')
        ...
        if cs:
            if isinstance(cs, esdl.DrivenByDemand):
                port = cs.get_inPort()
                if not port:
                    results.append('ERROR: Conversion ' + asset_id + ' has a DrivenByDemand strategy without an InPort')
            if isinstance(cs, esdl.DrivenBySupply):
                port = cs.get_inPort()
                if not port:
                    results.append('ERROR: Conversion ' + asset_id + ' has a DrivenBySupply strategy without an OutPort')
            if isinstance(cs, esdl.DrivenByProfile):
                prof = cs.get_profile()
                if not prof:
                    results.append('ERROR: Conversion ' + asset_id + ' has a DrivenByProfile strategy without a profile')
            if isinstance(cs, esdl.StorageStrategy):
                results.append('ERROR: Conversion ' + asset_id + ' has a StorageStrategy attached to it')
            if isinstance(cs, esdl.CurtailmentStrategy):
                results.append('ERROR: CurtailmentStrategy not supported yet')
        else:
            results.append('WARNING: Conversion ' + asset_id + ' has no control strategy')
