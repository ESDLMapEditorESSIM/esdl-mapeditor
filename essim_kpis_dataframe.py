...
class ESSIM_KPIs:
    ...
    def connect_to_database(self):
        self.database_client = DataFrameClient(host=self.config['ESSIM_database_server'], port=self.config['ESSIM_database_port'], database=self.scenario_id)
    ...
    def get_total_production_per_carrier(self):
        ...
        try:
            query = 'SELECT sum("allocationEnergy") FROM /' + self.es.get_name() + '.*/ WHERE (time >= \'' + self.start_date + '\' AND time < \'' + self.end_date + '\' AND "simulationRun" = \'' + self.simulationRun + '\' AND "assetType" = \'Producer\') GROUP BY fuelType'
            print(query)
    ...
    def get_total_consumption_per_carrier(self):
        ...
        try:
            # query = 'SELECT sum("allocationEnergy") FROM /' + self.es.get_name() + '.*/ WHERE (time >= \'' + self.start_date + '\' AND time < \'' + self.end_date + '\' AND "simulationRun" = \'' + self.simulationRun + '\' AND "assetType" = \'Consumer\') GROUP BY fuelType'
